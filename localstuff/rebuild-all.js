// Full pipeline: cleanup -> convert -> sync to client cache -> rebuild client
...
step('4. Sync server to client cache', () => {
	...
	console.log('Copying server to client cache...');
	...
	try {
		...
		// robocopy returns non-zero exit codes for success too. ignore.
	}
});

step('5. Rebuild client data files', () => {
	execSync(NODE + ' build full', { cwd: CLIENT_DIR, stdio: 'inherit', env: ENV });
});
...
console.log('\n=== ALL DONE ===');
console.log('Now restart your server and refresh the client browser tab.');

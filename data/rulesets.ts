...
export const Rulesets: import('../sim/dex-formats').FormatDataTable = {
	...
	newdawnrules: {
		...
		name: 'New Dawn Rules',
		ruleset: [
			'Standard AG', 'Sleep Clause Mod', 'Species Clause', 'OHKO Clause', 'Dynamax Clause',
			'New Dawn Item Clause', 'New Dawn Tier Clause', 'New Dawn Evasion Accuracy Clause',
		],
		banlist: ['Baton Pass', 'Shed Tail', 'Last Respects', 'King\'s Rock', 'Razor Fang'],
	},
	newdawnitemclause: {
		effectType: 'ValidatorRule',
		name: 'New Dawn Item Clause',
		onValidateTeam(team) {
			...
			for (const [itemid, count] of itemTable) {
				const limit = itemid === 'focussash' ? 1 : 2;
				if (count > limit) {
					return [
						`You are limited to ${limit} ${limit === 1 ? 'copy' : 'copies'} of ${this.dex.items.get(itemid).name} by New Dawn Item Clause.`,
					];
				}
			}
		},
	...
	newdawntierclause: {
		effectType: 'ValidatorRule',
		name: 'New Dawn Tier Clause',
		onValidateTeam(team, format) {
			let limited = 0;
			let semiLimited = 0;
			const doubles = format.gameType === 'doubles';
			for (const set of team) {
				const species = this.getValidationSpecies(set).tierSpecies;
				const tier = doubles ? species.doublesTier : species.tier;
				if (tier === 'LIM') limited++;
				if (tier === 'SEMI') semiLimited++;
			}
			if (limited > 1) {
				return [`You are limited to 1 Limited Pokémon by New Dawn Tier Clause.`];
			}
			if (limited + semiLimited > 2) {
				return [`You are limited to 2 total Limited/Semi-Limited Pokémon by New Dawn Tier Clause.`];
			}
		},
	},
	newdawnevasionaccuracyclause: {
		effectType: 'ValidatorRule',
		name: 'New Dawn Evasion Accuracy Clause',
		banlist: ['Sand Veil', 'Snow Cloak', 'Tangled Feet', 'Bright Powder', 'Lax Incense'],
		onValidateSet(set) {
			...
			for (const moveName of set.moves || []) {
				...
				if (move.category !== 'Status' || !boostKeys.length) continue;
				if (boostKeys.some(stat => !['accuracy', 'evasion'].includes(stat))) continue;
				if ((boosts.accuracy && boosts.accuracy < 0) || boosts.evasion) {
					problems.push(`${move.name} is banned by New Dawn Evasion Accuracy Clause.`);
				}
			}
			return problems;
		},
